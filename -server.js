...
app.post("/upload-multiple", upload.array("files"), (req, res) => {
  ...
  // 데이터베이스 연결 및 파일 정보 저장
  connectToDatabaseAndSaveFiles(files)
    .then(() => {
      res.json({ message: "파일 업로드 및 데이터베이스 저장 완료", files });
    })
    .catch((error) => {
      console.error("파일 정보 저장 에러", error);
      res
        .status(500)
        .json({ error: "파일 정보를 저장하는 중에 오류가 발생했습니다." });
    });
});
